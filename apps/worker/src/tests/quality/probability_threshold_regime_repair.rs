...
#[test]
fn regime_support_adjustment_keeps_20d_low_base_threshold_when_candidate_is_usable() {
    let mut rows = Vec::new();
    let mut probabilities = Vec::new();
    for _ in 0..20 {
        rows.push(threshold_regime_row(
    ...
    for index in 0..344 {
        ...
        probabilities.push(if index < 12 { 0.0047 } else { 0.0035 });
    }
    for index in 0..3557 {
        rows.push(threshold_regime_row(ProbabilityTrainingRegime::Normal, 0));
        probabilities.push(if index < 120 { 0.0046 } else { 0.0015 });
    }
    for _ in 0..45 {
        rows.push(threshold_regime_row(
            ProbabilityTrainingRegime::PostCrisisCooldown,
    ...
    let hit_rate = |threshold: f64, regime: ProbabilityTrainingRegime| {
        let total = row_refs
            .iter()
            .filter(|row| row.regime_20d == regime)
            .count() as f64;
        probabilities
            .iter()
            .zip(row_refs.iter())
            .filter(|(probability, row)| **probability >= threshold && row.regime_20d == regime)
            .count() as f64
            / total
    };

    assert!(adjusted_threshold < 0.005);
    assert!(
        hit_rate(
            adjusted_threshold,
            ProbabilityTrainingRegime::PreWarningBuffer
    ...
    assert!(
        hit_rate(
            adjusted_threshold,
            ProbabilityTrainingRegime::PreWarningBuffer
        ) > hit_rate(adjusted_threshold, ProbabilityTrainingRegime::Normal)
    );
    assert_eq!(
        hit_rate(
            adjusted_threshold,
