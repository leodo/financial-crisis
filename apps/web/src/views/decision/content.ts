export const decisionContent = {
  prelude: {
    ...
    dataCheckHint:
      "核验口径：当前概率卡片使用线上 active release；事件层的 0 表示当天没有规则命中；风险族 gate 的未触发表示该风险家族没有打开；发布审计里的历史/候选快照 0 不参与当前结论。",
  ...
  riskHorizon: {
    ...
    bandLegend:
      "5d 看急性冲击，20d 看未来几周是否需要离场和保护，60d 看数月级脆弱性；卡片里的距离进入线用于判断离准备/对冲/防守阈值还有多远。低位小概率会同时显示精确百分比、接口小数和概率基点；如果是小概率但非零，会明确标注，避免被粗略看成 0。",
    priorVsAction: {
      title: "先验和动作概率不是一回事",
      body:
