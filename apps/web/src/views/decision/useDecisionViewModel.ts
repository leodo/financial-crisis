...
export function useDecisionViewModel({
  ...
  const whyNowDrivers = useMemo(
    () => buildWhyNowDrivers(assessment, indicators),
...
type DriverTimingBucket = "near_term" | "recent" | "structural" | "stale" | "missing";
type WhyNowDriver = AssessmentSnapshot["top_risk_drivers"][number] & {
  timingBucket: DriverTimingBucket;
};

function buildWhyNowDrivers(
  assessment: AssessmentSnapshot,
  indicators: IndicatorRisk[]
  ...
  const baseDrivers = assessment.top_risk_drivers
    ...
    .filter((driver): driver is WhyNowDriver => driver !== null);
  const nearTermCandidates = indicators
    ...
    .filter((driver): driver is WhyNowDriver => driver !== null)
    .filter((driver) => driverPriority(driver.timingBucket) <= 1)
    .sort((left, right) => {
      const priorityDelta = driverPriority(left.timingBucket) - driverPriority(right.timingBucket);
      if (priorityDelta !== 0) {
  ...
  const selected = new Map<string, WhyNowDriver>();
  ...
  return Array.from(selected.values())
    ...
    .map(({ timingBucket: _timingBucket, ...driver }) => driver);
}
...
function driverPriority(timingBucket: DriverTimingBucket) {
  switch (timingBucket) {
    case "near_term":
    return 0;
    case "recent":
