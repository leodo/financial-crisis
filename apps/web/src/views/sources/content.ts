export const sourcesContent = {
  ...
  summaryNote: "先看关键指标覆盖、生产源健康降级和日期口径，再看每个源的使用建议；源健康分只说明抓取/源状态，不等同于当前结论可信度。",
  warningsEmpty: "当前没有额外数据可信度告警。",
  tableNote: "小屏幕下这张表支持横向滚动；最新观测、观测滞后、抓取水位和最近成功刷新是不同口径，使用建议说明该源能否参与当前判断，源健康分不是当前结论可信度。"
} as const;
