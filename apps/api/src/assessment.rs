...
use fc_domain::{
    ...
    AssessmentSnapshot, BacktestPerformanceSummary, BacktestScenarioSummary, DataMode, DataTrust,
    ...
    Observation, PositionGuidance, PostureGuidance, ProbabilityBlock, QualityGrade,
...
fn build_backtest_summary(backtests: &[BacktestScenarioSummary]) -> BacktestPerformanceSummary {
    ...
    let summary = format!(
        "当前内置场景回测覆盖 {} 个危机样本，至少提前 7 天给出有效预警的比例约为 {:.0}%。",
        scenario_count,
