...
pub fn actionable_warning_point(
    ...
    let strict_now_p5d_threshold = thresholds
        .map(|thresholds| thresholds.defend_p5d)
        .unwrap_or(0.18)
        .max(0.18);
    let strict_weeks_p20d_threshold = thresholds
        .map(|thresholds| thresholds.hedge_p20d)
        .unwrap_or(0.25)
        .max(0.25);
    let strict_short_horizon_signal = matches!(point.posture, DecisionPosture::Defend)
        || (matches!(point.posture, DecisionPosture::Hedge)
            && point.p_20d >= STRICT_HEDGE_POSTURE_P20D_FLOOR)
        || (matches!(point.time_to_risk_bucket, TimeToRiskBucket::Now)
            && point.overall_score >= 60.0
            && point.p_5d >= strict_now_p5d_threshold)
        || (matches!(point.time_to_risk_bucket, TimeToRiskBucket::Weeks)
            && point.overall_score >= 58.0
            && point.p_20d >= strict_weeks_p20d_threshold
    ...
    let standard_probability_plateau_prepare_signal = probability_plateau_prepare_setup
        ...
        && saturated_plateau_confirmation(point, thresholds)
        && saturated_action_context_confirmed(point, thresholds);
    ...
    let history_hysteresis_months_signal =
        matches!(point.time_to_risk_bucket, TimeToRiskBucket::Months)
            ...
            && point.p_20d
                >= thresholds
                    .map(|thresholds| thresholds.hedge_p20d)
                    .unwrap_or(STRICT_HISTORY_HYSTERESIS_MONTHS_P20D_FLOOR)
            ...
            && saturated_action_context_confirmed(point, thresholds)
            && history_hysteresis_months_context_supported(point)
...
pub fn strict_prepare_p20d_threshold(thresholds: Option<ActionableGateThresholds>) -> f64 {
    thresholds
        .map(|thresholds| {
            (thresholds.external_prepare_p20d * STRICT_PREPARE_P20D_THRESHOLD_RATIO)
                .clamp(
                    STRICT_PREPARE_P20D_THRESHOLD_MIN,
                    LEGACY_STRICT_PREPARE_P20D_THRESHOLD,
                )
                .max(thresholds.hedge_p20d)
        })
        ...
        .map(|thresholds| {
            (thresholds.prepare_p60d + STRICT_PREPARE_P60D_THRESHOLD_BUFFER)
                ...
                .clamp(
                    ...
                    thresholds
                        .prepare_p60d
                        .max(LEGACY_STRICT_PREPARE_P60D_THRESHOLD),
                )
        })
...
fn saturated_plateau_confirmation(
    point: &AssessmentHistoryPoint,
    thresholds: Option<ActionableGateThresholds>,
) -> bool {
    !thresholds.is_some_and(|thresholds| thresholds.prepare_p60d < 0.30)
        || !matches!(point.time_to_risk_bucket, TimeToRiskBucket::Months)
        || point.p_60d < STRICT_SATURATED_MONTHS_P60D_FLOOR
        || point.external_shock_score >= STRICT_SATURATED_PLATEAU_EXTERNAL_FLOOR
        || point.posture_trigger_codes.iter().any(|code| {
            matches!(
                code.as_str(),
                "prepare_external_structural" | "prepare_carry_structural"
...
fn history_hysteresis_months_context_supported(point: &AssessmentHistoryPoint) -> bool {
    probability_plateau_trigger_code(point)
        || point.posture_trigger_codes.iter().any(|code| {
            matches!(
                code.as_str(),
    ...
    thresholds
        .map(|thresholds| {
            (thresholds.hedge_p20d + STRICT_PREPARE_PLATEAU_P20D_BUFFER).clamp(
                ...
                thresholds.hedge_p20d.max(STRICT_PREPARE_PLATEAU_P20D_MAX),
            )
        })
...
fn actionable_history_hysteresis_months_structural_carry_signal(
    ...
    thresholds.is_some()
        ...
        && point.p_20d
            >= thresholds
                .map(|thresholds| thresholds.hedge_p20d)
                .unwrap_or(STRICT_HISTORY_HYSTERESIS_MONTHS_STRUCTURAL_CARRY_P20D_FLOOR)
                .max(STRICT_HISTORY_HYSTERESIS_MONTHS_STRUCTURAL_CARRY_P20D_FLOOR)
        && point.p_60d
            >= strict_prepare_p60d_threshold
...
mod tests {
    ...
    #[test]
    fn strict_prepare_thresholds_preserve_fail_closed_formal_floors() {
        let thresholds = ActionableGateThresholds {
            prepare_p60d: 0.99,
            hedge_p20d: 0.99,
            defend_p5d: 0.99,
        ...
        assert_eq!(strict_prepare_p20d_threshold(Some(thresholds)), 0.99);
        assert_eq!(strict_prepare_p60d_threshold(Some(thresholds)), 0.99);
    }
    ...
    fn saturated_plateau_requires_external_confirmation() {
        ...
        point.posture_trigger_codes = vec!["prepare_probability_plateau".to_string()];
    ...
    #[test]
    fn hedge_posture_requires_p20d_confirmation() {
        let thresholds = ActionableGateThresholds {
            prepare_p60d: 0.203,
            hedge_p20d: 0.06,
            defend_p5d: 0.99,
        ...
        point.p_20d = 0.29;
        point.p_60d = 0.93;
        point.overall_score = 57.9;
        point.external_shock_score = 43.9;
        point.posture_trigger_codes = vec!["hedge_p20d_context".to_string()];
    ...
    #[test]
    fn history_hysteresis_months_requires_context_support() {
        let thresholds = ActionableGateThresholds {
            prepare_p60d: 0.203,
            hedge_p20d: 0.06,
            defend_p5d: 0.99,
        ...
        point.p_20d = 0.37;
        point.p_60d = 0.66;
        point.overall_score = 44.0;
        point.external_shock_score = 37.0;
        ...
        point
            .posture_trigger_codes
            .push("prepare_probability_plateau".to_string());
        assert!(actionable_warning_point(&point, false, Some(thresholds)));
    }
    ...
    #[test]
    fn formal_fail_closed_thresholds_block_plateau_and_history_bypasses() {
        let thresholds = ActionableGateThresholds {
            prepare_p60d: 0.99,
            hedge_p20d: 0.99,
            defend_p5d: 0.99,
        ...
        let mut weeks_point = base_point();
        weeks_point.posture = DecisionPosture::Prepare;
        weeks_point.time_to_risk_bucket = TimeToRiskBucket::Weeks;
        weeks_point.p_20d = 0.55;
        weeks_point.p_60d = 0.93;
        weeks_point.overall_score = 62.0;
        weeks_point.external_shock_score = 45.0;
        weeks_point.posture_trigger_codes = vec![
            "prepare_probability_plateau".to_string(),
            "prepare_history_hysteresis".to_string(),
        ];

        let mut months_point = weeks_point.clone();
        months_point.time_to_risk_bucket = TimeToRiskBucket::Months;
        months_point.p_20d = 0.55;
        months_point.p_60d = 0.93;
        months_point.posture_trigger_codes = vec!["prepare_history_hysteresis".to_string()];

        assert!(!actionable_warning_point(
            &weeks_point,
            false,
            Some(thresholds)
        ));
