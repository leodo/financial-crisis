...
截至 `2026-05-31`，代码侧已经落实以下约束：

- `us_real_estate_home_price` 不再直接按房价指数水平评分，而是按 `12m同比` 评分。
- `us_external_usdjpy_level` 不再因为绝对汇率水平高就接近满分，而是按 `20d振幅` 评分。
- `rising_fast_is_riskier / falling_fast_is_riskier` 不再偷懒按原始水平评分，而是改成先算变化幅度，再做历史分位。
- 对“方向明确”的变化信号，如果当前没有进入风险方向，则单指标风险分直接记 `0`，避免 `EFFR / SOFR / JP call rate / M2` 在平稳期被误抬高。
- 前端指标表必须展示 `评分口径`，不能只给一个脱离上下文的风险分。
