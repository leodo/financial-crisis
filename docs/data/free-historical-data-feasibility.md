...
- `1990` 是统一日频多因子基线，不是历史数据上限；`1987` 可通过代理特征包纳入扩展场景，但不应假装与 `1990+` 面板完全等价。
...
| 数据 | 免费入口 | 代表性起点 | 适用说明 |
|---|---|---|---|
| `FEDFUNDS` | FRED Graph CSV | `1954-07` | 长历史利率主轴 |
| `BAA10Y` | FRED Graph CSV | `1986-01-02` | `1987` 可用的信用压力代理 |
| `VIXCLS` | FRED Graph CSV | `1990-01-02` | 也是第一阶段快变量统一基线的重要原因 |
| `DEXJPUS` | FRED Graph CSV | `1971-01-04` | `USDJPY` 长历史可覆盖 `1987` |
| Treasury Daily Curve | Treasury XML | `1990-01-02` | 日频国债曲线统一起点 |
| World Bank 年频数据 | World Bank API | 常见为 `1960` | 慢变量背景，不适合日频触发 |
| SEC EDGAR metadata | SEC API | 实务上 `1994+` | 事件层无法回到 `1987` |

因此最务实的判断是：

1. `1990+` 适合作为正式日频主回测区间。
2. `1987` 可以纳入，但应走“扩展场景 + 代理特征”路线。
3. `1960-1989` 更适合作为慢变量背景和历史类比补充，不适合强行做完整统一面板监督学习。
