...
- `actionability.rs` 只负责动作头子模块导出；`actionability/train.rs` 负责 actionability bundle/level 训练装配，`actionability/threshold.rs` 负责阈值选择与 calibration 择优，`actionability/guardrails.rs` 负责质量门禁策略与回归判定，`actionability/summary.rs` 负责动作 episode 评估摘要；`probability.rs`、`model.rs`、`training.rs`、`formal.rs` 负责训练、特征、数据集构建与共享数学/标签逻辑；其中 `training.rs` 还承接 formal bundle 训练管线和 `forward_crisis` 标签 / regime helper。
