...
- `scenario.rs` 负责 scenario 子模块导出壳层；`scenario/models.rs` 负责 `CrisisScenario`、`ActionEpisodePhase`、`ActionEpisodeSelection`，`scenario/timing.rs` 负责 action episode window / phase 与共享日期 helper，`scenario/episodes.rs` 负责 actionability level 映射、action window label、dominant episode 与 protected context phase，`scenario/horizon.rs` 负责 horizon support、anchor/start label helper，`scenario/selection.rs` 负责 primary / forward scenario 选择。
...
- `actionability.rs` 只负责动作头子模块导出；`actionability/train.rs` 负责 actionability bundle/level 训练装配，`actionability/threshold.rs` 负责阈值选择与 calibration 择优，`actionability/guardrails.rs` 负责质量门禁策略与回归判定，`actionability/summary.rs` 负责动作 episode 评估摘要；`probability.rs`、`model.rs`、`training.rs`、`formal.rs` 负责训练、特征、数据集构建与共享数学/标签逻辑；其中 `training.rs` 现已回到纯壳层导出，`training/types.rs` 负责训练数据 contract，`training/split.rs` 负责 chronological split 与 label-support gate，`training/regimes.rs` 负责 `forward_crisis` 标签 / regime helper，`training/pipeline.rs` 负责 formal bundle 训练、actionability 可选训练与 bundle/manifests/evaluation 写盘。
- `probability.rs` 负责概率主头训练、regime evaluation 与 bundle summary；`probability/threshold.rs` 负责 threshold 子模块导出与测试壳层，`probability/threshold/calibration.rs` 负责 calibration sample selection / calibration strategy，`probability/threshold/decision.rs` 负责 decision 子模块导出，`probability/threshold/decision/selection.rs` 负责 threshold selection、候选阈值与 score tuple，`probability/threshold/decision/regime.rs` 负责 regime-support threshold repair 与 early-warning gap guardrail，`probability/threshold/decision/metrics.rs` 负责 decision metrics 与 regime hit summary，`probability/threshold/diagnostics.rs` 负责 threshold diagnostics / calibration evidence；`probability/overlay.rs` 负责 family overlay 训练编排，`probability/overlay/tests.rs` 负责 family overlay support / split fallback 测试，`probability/overlay/audit.rs` 负责 overlay audit spec / audit 汇总，`probability/overlay/split/mod.rs` 负责 split 壳层与共享 support 结构，`probability/overlay/split/dataset.rs` 负责样本筛选与背景样本去重，`probability/overlay/split/balanced.rs` 负责 balanced fallback，`probability/overlay/split/bounds.rs` 负责 family-aware boundary search。
...
- `commands/backfill.rs` 负责 backfill 子模块导出；`commands/backfill/options.rs` 负责免费数据回填的通用时间窗/过滤/水位回退选项，以及 FRED 模式切换与 BOJ dataset 解析；`commands/backfill/execute.rs` 负责 execute 子模块导出壳层，`commands/backfill/execute/dispatch.rs` 负责 CLI source 分发，`commands/backfill/execute/market.rs` 负责 FRED / Treasury / World Bank / BOJ / JPY carry 这类 mapping 驱动的市场数据回填，`commands/backfill/execute/events.rs` 负责 GDELT / SEC EDGAR 这类带 payload/alert 的事件源回填，`commands/backfill/execute/shared.rs` 负责共享 seeded store 打开与 chunked mapping 执行器。
...
- `commands/snapshot.rs` 负责 prediction snapshot 子模块导出；`commands/snapshot/options.rs` 负责 snapshot query / export / dataset export 的 CLI 选项解析，`commands/snapshot/execute.rs` 负责 snapshot list/export/dataset research orchestration、snapshot 读取与默认 heuristic training release 选择，`commands/snapshot/render.rs` 负责 snapshot / training dataset 的 CSV/JSON 渲染与写盘 helper。
- `commands/pipeline.rs` 负责 pipeline 子模块导出；`commands/pipeline/options.rs` 负责 `--dataset-source` / `--model-shape` / bundle 输出目录 / release prefix 解析，`commands/pipeline/dataset.rs` 负责 dataset 子模块导出，`commands/pipeline/dataset/features.rs` 负责 transitional/formal feature helper，`commands/pipeline/dataset/formal.rs` 负责 formal dataset key 解析、formal dataset 读取/校验与 training row 映射，`commands/pipeline/dataset/snapshot.rs` 负责 snapshot 训练数据装配与 chrono split 前的 row build，`commands/pipeline/execute.rs` 负责 `train-probability` / `bootstrap-formal-release` 命令执行与控制台摘要打印。
...
- `commands/release/review.rs` 负责 release review orchestration 与子模块导出；`commands/release/review/options.rs` 负责 CLI 选项解析，`commands/release/review/snapshot.rs` 负责 runtime snapshot 抓取与 activate/restore helper，`commands/release/review/comparison.rs` 负责 release compare / runtime separation compare，`commands/release/review/summary.rs` 负责 recommendation 与 CLI summary 打印；`commands/release/review/focus.rs` 负责 focus 子模块导出，`commands/release/review/focus/backtest.rs` 负责 backtest scenario compare 辅助边界，`commands/release/review/focus/runtime.rs` 负责 runtime focus 子模块导出，`commands/release/review/focus/runtime/signals.rs` 负责 structured signal / actionable signal helper，`commands/release/review/focus/runtime/blocks.rs` 负责 runtime block 子模块导出壳层，`runtime/blocks/gating.rs` 负责 actionable block 分类与诊断文案，`runtime/blocks/facets.rs` 负责 posture/time bucket/facet helper 与 continuity facet 构造，`runtime/blocks/counts.rs` 负责 block/facet 计数汇总与 dominant category 选择，`runtime/blocks/failure.rs` 负责 primary failure mode 判定；`commands/release/review/focus/runtime/scenario/mod.rs` 负责 scenario focus diagnostics 壳层，`runtime/scenario/window.rs` 负责场景窗口准备与预危机切片，`runtime/scenario/points.rs` 负责 interesting point 选择与逐点 baseline/candidate 比较，`runtime/scenario/summary.rs` 负责最终诊断汇总。
- `release_review.rs` 负责 release review 报告结构、共享 formatter 与 Markdown 壳层；`release_review/historical/mod.rs` 负责 historical audit 子模块导出壳层，`release_review/historical/failure_modes.rs` 负责 failure mode summary，`release_review/historical/priorities.rs` 负责 historical audit priority 排序与 scenario catalog 映射，`release_review/historical/attribution.rs` 负责 attribution/action 汇总，`release_review/historical/workstreams.rs` 负责 workstream 聚合与 takeaways；`release_review/runtime/mod.rs` 负责 runtime 子模块导出壳层，`release_review/runtime/diagnostics.rs` 负责 runtime review 装配、regime scenario catalog fallback 与 posture/time-bucket 统计，`release_review/runtime/regimes.rs` 负责 runtime regime probability / separation diagnostics 与分类 helper，`release_review/runtime/takeaways.rs` 负责 runtime separation takeaways。
...
- `assessment.rs` 负责 assessment 总装配、method version 与 snapshot envelope；`assessment/posture.rs` 负责 posture 子模块导出，`assessment/posture/guidance.rs` 负责风险时距与姿态主装配，`assessment/posture/guidance/clauses.rs` 负责 posture clause 诊断，`assessment/posture/guidance/counters.rs` 负责确认计数规则，`assessment/posture/guidance/preferences.rs` 负责用户偏好升降级，`assessment/posture/position.rs` 负责仓位预算、动作手册、执行护栏和治理条款；`assessment/runtime_policy.rs` 负责 runtime threshold / serving policy / history runtime policy version；`assessment/common.rs` 负责 rounding、formatting、pressure 这类跨 assessment 子模块共享 helper；`assessment/tests/mod.rs` 负责 assessment 测试聚合，`assessment/tests/actionability.rs`、`time_bucket.rs`、`position.rs` 分别承接动作置信度、风险时距与 position guidance 测试，`assessment/tests/posture/mod.rs` 负责 posture 测试聚合，`posture/prepare.rs` 与 `posture/hedge.rs` 分别承接 prepare / hedge 场景断言，避免测试层重新回到单文件堆叠。
- `assessment/context.rs` 负责解释层上下文子模块导出；`assessment/context/runtime.rs` 负责 runtime freshness metadata，`assessment/context/indicators.rs` 负责关键指标新鲜度状态卡，`assessment/context/events.rs` 负责事件确认层解释，`assessment/context/backtests.rs` 负责 backtest summary 与空 rolling-audit fallback，`assessment/context/analogs.rs` 负责历史类比与相似度说明。
...
- `lib.rs` 只负责 API 入口、router 装配、cache middleware、refresh loop 与 tracing/shutdown 壳层；`tests.rs` 负责 `/health`、assessment、backtest、audit 与 reload 端点的第一层集成测试，避免入口运行时代码和接口测试继续混在同一个文件里。
- `handlers.rs` 只负责轻量 query type、日期解析、常规只读 handler 与 reload 入口；`handlers/research_audit.rs` 负责 release review 工件反序列化、SQLite release/snapshot/replay audit 装配与 research audit handler，避免文件扫描、SQLite audit 装配和普通 handler 继续堆在同一个入口文件里。
- `demo_seed.rs` 只负责 demo seed 子模块导出；`demo_seed/indicators.rs` 负责静态指标定义，`observations.rs` 负责演示观测序列与共享 series helper，`sources.rs` 负责 demo/runtime 数据源健康样本，`alerts.rs` 负责 demo alert 构造与最近告警筛选；`demo.rs` 只消费这些 seed API，不再内联样本细节；`demo/tests.rs` 负责 demo/runtime bridge 相关测试，避免运行时代码与测试再次堆在同一个文件里。
...
- `crates/storage/src/sqlite/tests/historical_replay/mod.rs` 负责 replay 存储测试聚合；`fixtures.rs` 负责 release / replay run / assessment point 样本构造，`runs.rs` 负责 replay run lookup/list 断言，`points.rs` 负责 assessment point round-trip 与 diagnostics 断言。`crates/storage/src/sqlite/tests/formal_datasets/mod.rs` 负责 formal dataset 存储测试聚合；`fixtures.rs` 负责 feature snapshot / dataset / row 样本构造，`snapshots.rs` 负责 feature snapshot round-trip 断言，`rows.rs` 负责 formal dataset manifest / row round-trip 断言，避免存储测试继续回到单文件堆叠。
- `apps/web/src/format.ts` 只负责前端格式化子模块导出；`format/labels.ts` 负责标签子模块导出壳层，`format/labels/risk.ts` 负责风险等级、姿态、概率窗口、数据新鲜度与用户档位标签，`format/labels/source.ts` 负责 dimension/source/source health 等数据源标签，`format/labels/indicator.ts` 负责 dataset/frequency/unit/score basis 等指标元数据标签，`format/labels/release.ts` 负责 probability mode、runtime threshold、release/review status 标签，`format/narrative.ts` 负责人话解释文案与 license/method/audit note 翻译，`format/technical.ts` 负责 technical id / release id / file reference 压缩显示，`format/posture.ts` 负责 posture clause 中文说明，`format/value.ts` 负责数值、百分比、日期与时间轴标签格式化；页面层只消费格式化 API，不再内联这些映射细节。
- `apps/web/src/views/decision/builders.ts` 只负责决策页展示拼装子模块导出；`builderTypes.ts` 负责运行态卡片、关键指标、signal layer、历史类比与 rolling audit episode 等纯展示行模型类型，`buildersCore.ts` 负责 runtime prelude、hero metrics、风险档位、action plan、JPY carry 与 analog 等核心展示拼装，`buildersBacktests.ts` 负责 backtest summary、history coverage 与 rolling audit 展示拼装；`useDecisionViewModel.ts` 继续只做 hook 编排，不再回收大块字符串/表格行装配细节。
- `crates/scoring/src/lib.rs` 只负责评分层子模块导出；`engine.rs` 负责 `ScoringEngine`、观测分桶与 snapshot 总装配，`signal.rs` 负责 signal transform、percentile/manual rule 与 signal value 评分，`aggregation.rs` 负责 dimension 聚合与质量汇总，`narrative.rs` 负责 level reason / indicator explanation / rounding helper，`tests.rs` 负责评分层单元测试；避免评分引擎、信号计算、维度聚合和解释文案继续堆在一个文件里。
...
1. 模型主线优先，但**新增非小修功能不再允许直接塞进**：
   ...
   - `crates/storage/src/sqlite/seeds/indicator_catalog.rs`
