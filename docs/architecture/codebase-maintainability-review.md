...
- `main.rs` 体量已从约 `7.6k` 行进一步降到约 `6` 行；共享导出、默认配置常量与测试聚合入口已迁入新的 `apps/worker/src/lib.rs`，`main.rs` 回到纯 Tokio 入口壳层；
- 因此，worker 当前的主要维护风险已从“所有 release 能力都堆在一个文件里”，下降为“运行时代码已基本按边界收口，测试层也开始具备稳定模块边界；后续仍可继续把 cross-topic fixture 与少量共享导入继续收窄”。随着 `overlay`、`overlay/split`、`threshold`、`focus`、`focus/runtime`、`review`、`release_review/runtime`、`release_review/historical`、`reporting/release_review`、`dataset/report`、`pipeline`、`release`、`feature`、`snapshot`、`release/probability`、`dataset`、`backfill`、`release/probability/compare`、`assessment/posture`、`assessment/context`、`assessment/probability`、`web/format`、`assessment/tests`、`demo_seed`、`history_replay`、`backtest` 主文件进一步收口之后，原先仍偏大的 `crates/domain/src/scenario_catalog.rs` 也已继续拆成 `scenario_catalog.rs` + `scenario_catalog/{types,load,validate,tests}.rs`，把纯类型定义、目录加载/默认模板注入、合法性校验与嵌入目录测试重新收回独立边界；本轮又把 `crates/ingestion/src/connectors/sec_edgar.rs` 收口为纯导出壳层，并拆成 `sec_edgar/{connector,types,parse,aggregate,rules,tests}.rs`，把 SEC 公司抓取编排、payload/filing 结构、submission/archive 解析、日度聚合与告警构造、规则打分以及测试重新收回独立边界；随后又把 `crates/domain/src/probability_bundle.rs` 收口为纯导出壳层，并拆成 `probability_bundle/{features,scoring,types,tests}.rs`，把特征常量与派生特征解析、logistic/Platt/overlay 打分、bundle schema 与测试重新收回独立边界。当前 shared domain 层的大块热点已明显下降，后续重点更多转向持续观察少数中等体量运行时代码，而不是再处理一个 900 行级 shared 单文件。
