...
- 存储测试虽然已按主题拆开，但 `tests/historical_replay.rs`、`tests/formal_datasets.rs` 仍会继续承接较重的 round-trip 断言；
