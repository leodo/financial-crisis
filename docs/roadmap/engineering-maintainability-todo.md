...
- 已新增 `apps/worker/src/actionability.rs`，把 actionability bundle 训练、阈值选择、校准策略、guardrail 与 actionability evaluation summary 从 `main.rs` 中拆出，供训练与 release review 共用；本轮又继续细分为 `actionability.rs` + `actionability/{train,threshold,guardrails,summary}.rs`，父文件已收口为模块壳层。
...
- `apps/worker/src/release_review.rs` 本轮继续瘦身：historical audit 的 failure mode / priority / attribution / action / workstream 汇总与 takeaways 先拆到 `apps/worker/src/release_review/historical.rs`，随后又继续细分为 `historical/{mod,failure_modes,priorities,attribution,workstreams}.rs`；runtime regime probability / separation diagnostics、分类 helper 与 runtime takeaways 先拆到 `apps/worker/src/release_review/runtime.rs`，随后又继续细分为 `runtime/{mod,diagnostics,regimes,takeaways}.rs`；主文件开始回到“report wire structs + shared formatter + Markdown 壳层”边界。
...
- `apps/worker/src/scenario.rs` 本轮继续拆成 `scenario.rs` + `scenario/{models,timing,episodes,horizon,selection}.rs`，父文件已从约 `489` 行收缩到约 `15` 行；`models.rs` 收走 `CrisisScenario` / `ActionEpisodePhase` / `ActionEpisodeSelection`，`timing.rs` 收走 action episode window / phase 与共享日期 helper，`episodes.rs` 收走 actionability level 映射、action window label、dominant episode 与 protected context phase，`horizon.rs` 收走 horizon support / anchor / action window helper，`selection.rs` 收走 primary / forward scenario 选择。场景时间窗逻辑已不再重新堆回单文件。
...
- 这一轮之后，`apps/worker/src/main.rs` 已从约 `7,573` 行继续收缩到约 `6` 行；共享导出、默认配置常量与测试聚合入口已迁入新的 `apps/worker/src/lib.rs`，`main.rs` 当前只保留 Tokio 初始化、参数收集与 `run_from_args` 转发。
- 当前 `main.rs` 已不再直接承载训练管线、release review 报告结构、动作 episode / scenario 时间窗逻辑、通用 API/IO helper、共享导出、默认配置常量，也不再直接承载超大测试块；worker 入口已正式回到纯 CLI 壳层。
...
- `apps/worker/src/commands/backfill.rs` 本轮也继续拆成 `backfill.rs` + `backfill/options.rs` + `backfill/execute.rs`，主文件已从约 `738` 行收缩到约 `11` 行；免费数据回填的通用时间窗/过滤/水位回退选项、FRED 模式切换与 BOJ dataset 解析已拆到 `options.rs`。随后原先约 `500` 行的 `execute.rs` 又继续细分为 `execute/{dispatch,market,events,shared}.rs`，把 CLI source 分发、市场型 mapping 回填、GDELT/SEC 事件源回填、共享 seeded store 与 chunked mapping 执行器重新收回独立边界；父文件现已收口到约 `10` 行。随后原先约 `451` 行的 `apps/worker/src/commands/release/review/focus/runtime/scenario.rs` 又继续细分为 `runtime/scenario/{mod,window,points,summary}.rs`，把场景窗口准备、interesting point 比较与最终诊断汇总重新收回独立边界；父文件现已收口到约 `75` 行。随后原先约 `432` 行的 `apps/worker/src/commands/release/review/focus/runtime/blocks.rs` 又继续细分为 `runtime/blocks/{gating,facets,counts,failure}.rs`，把 block 分类与诊断文案、continuity facet 构造、block/facet 计数汇总与 primary failure mode 判定重新收回独立边界；随后原先约 `401` 行的 `apps/worker/src/commands/snapshot.rs` 又继续细分为 `snapshot/{options,execute,render}.rs`，把 query/export 参数解析、research orchestration 与 CSV/JSON 渲染重新收回独立边界；随后原先约 `406` 行的 `apps/api/src/assessment/context.rs` 又继续细分为 `context/{runtime,indicators,events,backtests,analogs}.rs`，把 runtime freshness、关键指标状态、事件确认、backtest summary 和历史类比分别收回独立边界；随后原先约 `411` 行的 `apps/worker/src/commands/pipeline/dataset.rs` 又继续细分为 `dataset/{features,formal,snapshot}.rs`，把 transitional/formal feature helper、formal dataset key/读取校验与 snapshot 训练数据装配重新收回独立边界；随后原先内联在 `apps/worker/src/probability/overlay.rs` 里的 family overlay 测试又整体外移到 `overlay/tests.rs`，让运行时代码回到约 `131` 行纯训练编排壳层；本轮又把原先约 `473` 行的 `crates/domain/src/scenario_catalog.rs` 继续细分为 `scenario_catalog/{types,load,validate,tests}.rs`，把场景目录类型、加载回退、默认 action episode 模板注入、引用合法性校验和嵌入目录测试重新收回独立边界；随后又把原先约 `965` 行的 `crates/ingestion/src/connectors/sec_edgar.rs` 继续细分为 `sec_edgar/{connector,types,parse,aggregate,rules,tests}.rs`，把 SEC 免费事件源的抓取编排、submission/archive 解析、规则打分、日度聚合和测试重新收回独立边界；本轮再把原先约 `981` 行的 `crates/domain/src/probability_bundle.rs` 继续细分为 `probability_bundle/{features,scoring,types,tests}.rs`，把 shared feature 常量与派生 resolver、logistic/Platt/overlay 打分、bundle schema 和 shared tests 重新收回独立边界。当前 shared 层最高风险的大文件已基本收口，下一阶段治理可转向持续观察中等体量运行时代码，而不是继续围绕少数 900 行级共享文件救火。
- `apps/worker/src/commands/release/probability/compare.rs` 本轮也继续拆成 `compare.rs` + `compare/build.rs` + `compare/render.rs`，主文件已从约 `712` 行收缩到约 `141` 行；formal probability compare 的窗口对齐、阈值/命中统计、feature delta 聚合与窗口汇总已拆到 `build.rs`，JSON/CSV 写盘与 CLI 摘要打印已拆到 `render.rs`。随后治理优先热点一度切换到 `apps/api/src/assessment/posture.rs`，目前该轮高收益拆分也已完成。
- `apps/worker/src/model.rs` 的高收益拆分已基本完成；`apps/worker/src/probability/overlay.rs` 本轮也已进一步拆成 `overlay.rs` + `overlay/audit.rs` + `overlay/split.rs`，主文件从约 `1260` 行收缩到约 `438` 行；随后 `overlay/split.rs` 又继续细分为 `overlay/split/{mod,dataset,balanced,bounds}.rs`，把数据集构建、balanced fallback、family-aware boundary search 和共享 support 结构重新收回独立边界；本轮又把内联测试整体外移到 `overlay/tests.rs`，让运行时代码回到约 `131` 行纯编排壳层。`apps/worker/src/probability/threshold.rs` 本轮也已进一步拆成 `threshold.rs` + `threshold/calibration.rs` + `threshold/decision.rs` + `threshold/diagnostics.rs`，主文件从约 `1080` 行收缩到约 `108` 行；随后原先约 `496` 行的 `apps/worker/src/probability/threshold/decision.rs` 又继续细分为 `decision/{selection,regime,metrics}.rs`，把阈值候选/打分、regime-support 修复与 decision metrics/regime hit 汇总重新收回独立边界。`apps/worker/src/actionability.rs` 本轮也继续拆成 `actionability.rs` + `actionability/{train,threshold,guardrails,summary}.rs`，把动作头训练装配、阈值/校准择优、质量门禁与 episode 评估摘要重新收回独立边界。`apps/worker/src/training.rs` 本轮也继续拆成 `training.rs` + `training/{types,split,regimes,pipeline}.rs`，把训练数据 contract、chronological split、`forward_crisis` 标签 / regime helper 和 formal bundle 训练写盘重新收回独立边界，父文件从约 `553` 行收缩到约 `18` 行。`apps/worker/src/commands/release/review/focus.rs` 本轮也已进一步拆成 `focus.rs` + `focus/backtest.rs` + `focus/runtime.rs`，主文件从约 `1072` 行收缩到约 `6` 行；随后 `focus/runtime.rs` 又继续拆成 `runtime.rs` + `runtime/signals.rs` + `runtime/blocks.rs` + `runtime/scenario.rs`，主文件从约 `987` 行收缩到约 `5` 行；随后原先约 `432` 行的 `runtime/blocks.rs` 又继续细分为 `runtime/blocks/{gating,facets,counts,failure}.rs`，把 block 分类与诊断文案、continuity facet 构造、block/facet 计数汇总与 primary failure mode 判定重新收回独立边界；随后原先约 `451` 行的 `runtime/scenario.rs` 又继续细分为 `runtime/scenario/{mod,window,points,summary}.rs`，把场景窗口准备、interesting point 比较与最终诊断汇总重新收回独立边界；`apps/worker/src/commands/release/review.rs` 本轮也继续拆成 `options.rs`、`snapshot.rs`、`comparison.rs`、`summary.rs` 四个子模块，主文件从约 `893` 行收缩到约 `258` 行；`apps/worker/src/commands/release.rs` 本轮也继续拆成 `options.rs`、`lifecycle.rs`、`guardrails.rs` 三个子模块，主文件从约 `774` 行收缩到约 `35` 行；`apps/worker/src/commands/release/probability.rs` 本轮也继续拆成 `options.rs`、`execute.rs` 两个子模块，主文件从约 `679` 行收缩到约 `15` 行；`apps/worker/src/commands/feature.rs` 本轮也继续拆成 `options.rs`、`snapshot.rs`、`visibility.rs`、`coverage.rs` 四个子模块，主文件从约 `727` 行收缩到约 `16` 行；`apps/worker/src/commands/snapshot.rs` 本轮也继续拆成 `options.rs`、`execute.rs`、`render.rs` 三个子模块，主文件从约 `401` 行收缩到约 `10` 行；`apps/worker/src/reporting/release_review.rs` 本轮也继续拆成 `overview.rs`、`historical.rs`、`focus.rs`、`diagnostics.rs` 四个子模块，主文件从约 `854` 行收缩到约 `54` 行；`apps/worker/src/release_review/runtime.rs` 本轮又继续细分为 `runtime/{mod,diagnostics,regimes,takeaways}.rs`，把 runtime review 装配、regime 分析和 takeaways 重新收回独立边界；`apps/worker/src/release_review/historical.rs` 本轮又继续细分为 `historical/{mod,failure_modes,priorities,attribution,workstreams}.rs`，把 failure summary、priority 排序、attribution/action 汇总和 workstream/takeaways 重新收回独立边界；`apps/worker/src/commands/dataset/report.rs` 本轮也继续拆成 `summary.rs`、`slice.rs`、`render.rs` 三个子模块，主文件从约 `834` 行收缩到约 `105` 行；`apps/worker/src/commands/pipeline.rs` 本轮也继续拆成 `options.rs`、`dataset.rs`、`execute.rs` 三个子模块，主文件从约 `830` 行收缩到约 `13` 行；随后原先约 `411` 行的 `apps/worker/src/commands/pipeline/dataset.rs` 又继续细分为 `dataset/{features,formal,snapshot}.rs`，把 transitional/formal feature helper、formal dataset key/读取校验与 snapshot 训练数据装配重新收回独立边界；`apps/worker/src/commands/dataset.rs` 本轮也继续拆成 `options.rs`、`build.rs`、`execute.rs` 三个子模块，主文件从约 `676` 行收缩到约 `35` 行；`apps/worker/src/commands/backfill.rs` 本轮也继续拆成 `options.rs`、`execute.rs` 两个子模块，主文件从约 `738` 行收缩到约 `11` 行；随后 `apps/worker/src/commands/backfill/execute.rs` 又继续细分为 `execute/{dispatch,market,events,shared}.rs`，把 source 分发、市场型 mapping 回填、事件源回填和共享 seeded store / chunk executor 重新收回独立边界；`apps/worker/src/commands/release/probability/compare.rs` 本轮也继续拆成 `build.rs`、`render.rs` 两个子模块，主文件从约 `712` 行收缩到约 `141` 行；`apps/worker/src/scenario.rs` 本轮也继续拆成 `scenario/{models,timing,episodes,horizon,selection}.rs`，把场景时间窗相关的模型、phase/timing、label/protected context、horizon helper 与 primary/forward scenario 选择重新收回独立边界。随后 `crates/storage/src/sqlite/metadata.rs` 也已继续拆成 `metadata.rs` + `metadata/catalog.rs` + `metadata/mappings.rs`，并把 source/dataset/entity 注册逻辑改成结构化 seed helper；本轮又继续把 `sqlite/seeds.rs` 收口为 `seeds.rs` + `seeds/indicator_catalog.rs` + `seeds/mappings.rs`，把 `sqlite/tests.rs` 收口为 `tests/mod.rs` + 多个主题测试模块；`apps/api/src/demo.rs` 又继续把测试外移到 `demo/tests.rs`，`apps/api/src/assessment/posture/guidance.rs` 也继续拆成 `guidance.rs` + `guidance/{clauses,counters,preferences}.rs`。当前工程治理更真实的下一阶段热点已从少数 900 行级 shared model 文件转向持续观察中等体量运行时代码；`probability_bundle`、`sec_edgar` 与 `scenario_catalog` 这一轮高收益拆分已完成。
...
- `apps/api/src/assessment/context.rs` 本轮也继续拆成 `context.rs` + `context/{runtime,indicators,events,backtests,analogs}.rs`，主文件已从约 `406` 行收缩到约 `10` 行；runtime freshness metadata、关键指标新鲜度卡片、事件确认解释、backtest summary 与历史类比说明已各自落回独立边界。
...
- 已把原本约 `624` 行的 `apps/worker/src/tests/options.rs` 拆成 `apps/worker/src/tests/options/mod.rs` + `refresh.rs`、`release.rs`、`snapshots.rs`、`dataset.rs`、`pipeline.rs` 五个主题测试模块，降低 CLI / dataset / pipeline 参数测试继续堆成长文件的风险。
- 已把原本约 `1254` 行的 `apps/worker/src/tests/review.rs` 拆成 `apps/worker/src/tests/review/mod.rs` + `historical_audit.rs`、`runtime.rs` 与 `focus/mod.rs`；其中 `focus/mod.rs` 本轮又继续细分为 `comparison.rs`、`diagnostics.rs`、`continuity.rs`、`failure_modes.rs` 四个主题测试模块，降低 release review 测试随诊断逻辑扩展继续堆成长文件的风险。
...
- 已新增 `crates/storage/src/sqlite/` 子模块目录；除聚合子模块 `metadata.rs`、`observations.rs`、`operational.rs`、`releases.rs`、`prediction_snapshots.rs`、`feature_snapshots.rs`、`formal_datasets.rs`、`historical_replay.rs` 外，本轮又继续补齐 `helpers.rs`、`rows.rs`、`migrations.rs`、`seeds.rs` / `seeds/*`、`tests/mod.rs` / `tests/*`。
- `sqlite.rs` 当前已收缩为约 124 行壳层，只保留连接选项、store shell、共享常量/record type、模块导入与 `RiskStore` trait 转接；日期/ID helper、row mapper、迁移补列、seed 定义和 storage round-trip tests 已分别下沉到子模块。
...
- 当前工程治理优先热点已切换为仍偏大的少量 worker / web 运行时代码热点；共享训练模块 `apps/worker/src/training.rs` 的子模块化拆分已完成，formal dataset / historical replay 运行时代码与存储层重型测试主题的子模块化拆分也已完成，API 入口测试、posture 测试分层与 research audit handler 收口也已完成。
...
- [x] `apps/worker/src/main.rs` 不再承担所有主线职责。
