...
- `apps/worker/src/commands/backfill.rs` 本轮也继续拆成 `backfill.rs` + `backfill/options.rs` + `backfill/execute.rs`，主文件已从约 `738` 行收缩到约 `11` 行；免费数据回填的通用时间窗/过滤/水位回退选项、FRED 模式切换与 BOJ dataset 解析已拆到 `options.rs`。随后原先约 `500` 行的 `execute.rs` 又继续细分为 `execute/{dispatch,market,events,shared}.rs`，把 CLI source 分发、市场型 mapping 回填、GDELT/SEC 事件源回填、共享 seeded store 与 chunked mapping 执行器重新收回独立边界；父文件现已收口到约 `10` 行。当前更真实的下一阶段热点已转向 `crates/domain/src/probability_bundle.rs`、`crates/ingestion/src/connectors/sec_edgar.rs` 与 `apps/worker/src/commands/release/review/focus/runtime/scenario.rs` 这类仍偏大的共享逻辑文件。
