...
- [ ] 维护“每个页面关键数字从哪里来”的审计清单；没有来源、单位、日期和状态的数字不进入首页。

#### Iteration 1：页面数字可信

- [ ] 首页 `audit_only` 时，主标题、数字卡、风险时距、概率轨迹和动作建议必须一致地说明：formal 概率只作为审计读数，不是主结论。
- [ ] `release_status=healthy` 只能说明服务和 bundle 可加载，不能被展示成“模型结论可信”；页面必须同时展示 `model_input_status`。
- [ ] 低位小概率不得被舍入成容易误读的 `0`；必须显示精确百分比、基点和接口原始值。
- [ ] `20d` 偏冷、USDJPY tail 方向异常、formal 概率被降级时，UI 必须给出人能看懂的阻断原因。
- [ ] 结论可信度必须被 `audit_only` 和关键数据缺失/陈旧封顶，不能因为数据覆盖高就显示高可信。

#### Iteration 2：决策面板 MVP

- [ ] 首页首屏只回答四个问题：当前是否危险、风险更像数月/数周/当下/未知、为什么、当前应观察/准备/对冲/防守。
- [ ] 仓位建议保持系统预算口径：风险资产上限、现金目标、对冲覆盖、保护性期权比例；不得输出自动交易或“一键清仓”。
